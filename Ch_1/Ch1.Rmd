...
```
```{r echo=FALSE}
print("CLT says it should be about 0.05: Since 95% of data lies inside 2 SD from problem 2 in last chapter. To confirm:")
```
```{r}
2*pnorm(-2)
qqnorm(z)
...
```
```{r echo=FALSE}
print("D. Normal approximation works bes when p -> 0 and n-> inf")
```
...
```{r echo=FALSE}
print("A. Normal with mean 0 and SD 1.")
```
...
```{r echo=FALSE}
print("B. These are two different assumptions. The t-distribution accounts for the variability introduced by the estimation of the standard error and thus, under the null, large values are more probable under the null distribution.")
```
```{r echo=FALSE}
rm(list = ls())
```
...
```{r echo=FALSE}
print("C. N and thus the degrees of freedom is large enough to make the normal and t-distributions very similar.")
```
...
```
```{r echo=FALSE}
print("sample size 60 gives power of about 80%.")
```
...
```
```{r echo=FALSE}
print("sample size 90 gives power of about 80%.")
```
...
```
```{r echo=FALSE}
print("C. The approximations are spot on for all sample sizes.")
```
...
```{r echo=FALSE}
print("TRUE. t-distribution with 499 df is essentially normal. t-distribution assumes original population is normally distributed. When the samples are large enough the mean should be normally distributed around p (by definition of p and also by CLT). So this is just two normal distributions fitting.")
```
...
```
```{r echo=FALSE}
print("D. [Also, theoretically (http://davidmlane.com/hyperstat/sampling_dist.html0) the variance of sample median should be 1.253 times 1/sqrt(N)].")
```
```{r echo=FALSE}
rm(list = ls())
```
...
```
```{r echo=FALSE}
print("Since p-value is not significant, there is not much difference in case/control status due to allele.")
```
